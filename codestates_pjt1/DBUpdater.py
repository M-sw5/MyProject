...
import requests, time, calendar, json, urllib
from threading import Timer
from urllib.request import urlopen # 이 부분은 어디서 사용하는지 모르겠음.

class DBUpdater:
    def __init__(self):
        """생성자: MariaDB 연결 및 종목코드 딕셔너리 생성"""
        self.conn = pymysql.connect(host='localhost', user='root', password='Ssj312505!', db='INVESTAR', charset='utf8')
    ...
    def read_naver(self, code, company, pages_to_fetch):
        """네이버 금융에서 주식 시세를 읽어서 데어터프레임으로 반환"""
        try:
            ...
            df = df[['date','open','high','low','close','diff','open','volume']]
